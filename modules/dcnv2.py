...
class DeformableConv2dwithFwarpv2(nn.Module):
    ...
    def __init__(self, in_c, out_c, kernel_size=3, stride=1, padding=1, groups=8):
        ...
        self.offset_flow_conv = nn.Sequential(
            ...
            nn.Conv2d(in_c, in_c, 3, 1, 1),
            nn.PReLU(in_c),
            nn.Conv2d(in_c, 2, 3, 1, 1),
        )
        # Generate residual flow and modulator
        self.conv_offset_mask = nn.Sequential(
            nn.Conv2d(in_c * 2 + 2, in_c, 3, 1, 1),
            nn.PReLU(in_c),
            nn.Conv2d(in_c, in_c, 3, 1, 1),
            nn.PReLU(in_c),
            nn.Conv2d(in_c, groups * 3 * kernel_size * kernel_size, 3, 1, 1),
        )
        # self.conv_offset_mask = nn.Conv2d(in_channels * 2 + 2, groups * 3 * kernel_size * kernel_size, 3, 1, 1)
        self.conv_offset_mask.apply(self.init_zero)

        self.regular_conv = nn.Conv2d(in_channels=in_c // groups,
                                      out_channels=out_c,
                                      kernel_size=kernel_size,
                                      stride=1,
                                      padding=padding)
