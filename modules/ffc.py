...
class FFCTexture(nn.Module):
    def __init__(self, input_nc, n_downsampling=3, nfeats=(64, 128, 256, 256),
                 n_blocks=6, norm_layer=nn.BatchNorm2d, padding_type='reflect', activation_layer=nn.ReLU,
        ...
        super().__init__()
        assert n_downsampling == len(nfeats) - 1
        model = [nn.ReflectionPad2d(3),
                 FFC_BN_ACT(input_nc, nfeats[0], kernel_size=7, padding=0, norm_layer=norm_layer,
                            activation_layer=activation_layer, **init_conv_kwargs)]
        ...
        for i in range(n_downsampling):
            if i == n_downsampling - 1:
                ...
                cur_conv_kwargs = downsample_conv_kwargs
            model += [FFC_BN_ACT(nfeats[i], nfeats[i+1],
                                 kernel_size=3, stride=2, padding=1,
                                 norm_layer=norm_layer,
                                 activation_layer=activation_layer,
                                 **cur_conv_kwargs)]
        ...
        # resnet blocks
        for i in range(n_blocks):
            cur_resblock = FFCResnetBlock(nfeats[-1], padding_type=padding_type,
                                          activation_layer=activation_layer, norm_layer=norm_layer, **resnet_conv_kwargs)
            model += [cur_resblock]

        model += [ConcatTupleLayer()]
        model += [nn.Conv2d(nfeats[-1], nfeats[-1], 1, 1)]
        self.model = nn.Sequential(*model)
...
if __name__ == '__main__':
    model = FFCTexture(9, 3, n_blocks=4)
    num_params = sum(p.numel() for p in model.parameters())
    print('Number of params:', num_params)

    inp = torch.rand((1, 9, 256, 256))
    out = model(inp)
